...
4. **Branch** 选择 `main`，目录选择 `/html`，点击 Save。
5. 等待 1–2 分钟，访问 `https://<用户名>.github.io/<仓库名>/` 即可。

## 自定义

- 联系方式：站点内所有 X 链接统一指向 `https://x.com/2005yoo`，改品牌/文案直接编辑 `html/index.html`。
- 配色主题：在 `html/css/style.css` 顶部的 `:root` 变量中调整 `--gold`、`--bg` 等。
- 付费咨询定价：`html/index.html` 中 `#pricing` 区块的三个方案。
